## Hello, I'm Khagendra Neupane

## About Me

I am a **Cybersecurity and Networking professional** from Nepal 🇳🇵 with a passion for building secure, high-performance systems. My core expertise lies in Python, C/C++, and Linux systems, where I focus on creating practical tools for network programming and system security.

<details>
  <summary>🔎 <strong>See more details</strong></summary>
  <br/>
  - 🔭 I'm the creator of <strong><a href="https://pypi.org/project/kn-sock/">kn-sock</a></strong>, a Python library that simplifies socket programming.
  <br/>
  - 🌱 I'm currently deepening my skills in secure web application development with <strong>Flask</strong> and exploring <strong>AI/ML applications in cybersecurity</strong>.
  <br/>
  - 💬 I'm open to collaborating on open-source networking tools and ethical hacking projects. Let's build something amazing together!

  <h3 align="left">Connect & Follow</h3>
  <p align="left">
    <a href="https://www.khagendraneupane.com.np/" target="_blank">
      <img src="https://img.shields.io/badge/Portfolio-WebApp-blue?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Portfolio Website">
    </a>
    <a href="https://www.linkedin.com/in/khagendra-neupane-37427532a/" target="_blank">
      <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
    </a>
    <a href="mailto:nkhagendra1@gmail.com">
      <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email Me">
    </a>
  </p>
</details>

---

## 🚀 Featured Projects

<table>
<tr>
<td width="50%" valign="top">
  <h3 align="left">kn-sock: Simplified Socket Toolkit</h3>
  <br />
  <a href="https://github.com/KhagendraN/kn-sock" target="_blank">
    <img src="assets/kn-sock_logo.png" style="max-width: 300px; height: auto;" alt="kn-sock project preview">
  </a>
  <br />
  <p align="left">
    <a href="https://github.com/KhagendraN/kn-sock" target="_blank">
      <img src="https://img.shields.io/badge/View%20Code-181717?style=for-the-badge&logo=github&logoColor=white">
    </a>
    <a href="https://github.com/KhagendraN/kn-sock/blob/main/docs/index.md" target="_blank">
      <img src="https://img.shields.io/badge/Read%20Docs-00A65A?style=for-the-badge&logo=readme&logoColor=white">
    </a>
  </p>
  <p>
    <strong>Problem:</strong> Python's native socket library requires significant boilerplate code, making simple network applications unnecessarily complex.
    <br/><br/>
    <strong>Solution:</strong> A lightweight Python toolkit designed to abstract away these complexities. It provides a high-level API for creating network clients and servers, drastically simplifying the development of network tools.
  </p>
</td>
<td width="50%" valign="top">
  <h3 align="left">Jarvis Voice Assistant</h3>
  <br />
  <a href="https://github.com/KhagendraN/Jarvis-Voice-Assistant" target="_blank">
    <img src="assets/jarvis-removebg-preview.png" style="max-width: 300px; height: auto;" alt="Jarvis project preview">
  </a>
  <br />
  <p align="left">
    <a href="https://github.com/KhagendraN/Jarvis-Voice-Assistant" target="_blank">
      <img src="https://img.shields.io/badge/View%20Code-181717?style=for-the-badge&logo=github&logoColor=white">
    </a>
  </p>
  <p>
    <strong>Goal:</strong> To explore the integration of AI/ML libraries and external APIs for creating interactive, voice-controlled desktop applications.
    <br/><br/>
    <strong>Solution:</strong> A Python-based desktop assistant inspired by "Jarvis". It leverages speech recognition and text-to-speech engines to perform tasks like opening applications, fetching information from the web, and automating system functions.
  </p>
</td>
</tr>
</table>

<details>
  <summary>📦 <strong>View Additional Projects</strong></summary>
  <br/>
  <ul>
    <li><a href="https://github.com/KhagendraN/LANConnect-Chat-Application"><strong>LANConnect Chat App</strong></a>: A terminal-based chat application for local networks built with Python sockets and threading.</li>
    <li><a href="https://github.com/KhagendraN/ArchShell"><strong>ArchShell</strong></a>: A custom basic shell implementation written in C to explore Linux process management and system calls.</li>
    <li><a href="https://github.com/KhagendraN/simple-http-server"><strong>Simple HTTP Server</strong></a>: A foundational web server built from scratch to understand the core principles of the HTTP protocol.</li>
  </ul>
</details>

---

## 🛠️ My Tech Stack

<p align="center">
  <a href="https://www.python.org" target="_blank"> <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python"/> </a>
  <a href="https://en.wikipedia.org/wiki/C_(programming_language)" target="_blank"> <img src="https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=black" alt="C"/> </a>
  <a href="https://isocpp.org/" target="_blank"> <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white" alt="C++"/> </a>
  <a href="https://www.linux.org/" target="_blank"> <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" alt="Linux"/> </a>
  <a href="https://www.gnu.org/software/bash/" target="_blank"> <img src="https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white" alt="Bash"/> </a>
  <a href="https://flask.palletsprojects.com/" target="_blank"> <img src="https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white" alt="Flask"/> </a>
  <a href="https://www.docker.com/" target="_blank"> <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" alt="Docker"/> </a>
  <a href="https://www.vim.org/" target="_blank"> <img src="https://img.shields.io/badge/Vim-019733?style=for-the-badge&logo=vim&logoColor=white" alt="Vim"/> </a>
</p>

---

## 📈GitHub Stats
<details>
  <summary><strong>See My GitHub Stats</strong></summary>
  <br/>
  <p align="left">
    <img align="center" src="https://github-readme-stats.vercel.app/api?username=KhagendraN&theme=gruvbox&show_icons=true&hide_border=true&count_private=true" alt="Khagendra's GitHub Stats"/>
    <br/><br/>
    <img align="center" src="https://github-readme-stats.vercel.app/api/top-langs/?username=KhagendraN&layout=compact&theme=gruvbox&hide_border=true" alt="Khagendra's Top Languages"/>
    <br/><br/>
    <img align="center" src="https://streak-stats.demolab.com?user=KhagendraN&theme=gruvbox&hide_border=true" alt="Khagendra's GitHub Streak"/>
  </p>
</details>

<br/>

<p align="left">
  <img src="https://profile-counter.glitch.me/KhagendraN/count.svg" alt="Visitor Count">
</p>